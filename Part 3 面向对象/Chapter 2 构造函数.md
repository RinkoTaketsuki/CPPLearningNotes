...
2. 如果未显式定义任何构造函数，编译器会隐式定义一个默认构造函数（合成的默认构造函数），该构造函数的规则是：
    - 若存在类内初始值，使用该初始值初始化成员（有些编译器不支持类内初始值）
    - 否则默认初始化成员
3. 若在类内显式定义了非默认构造函数，则编译器不会再合成默认构造函数，此时对象无法默认初始化
4. 若类内有无默认构造函数的类型的成员，则无法合成默认构造函数
5. 如果在 3 的情况下还是需要合成的默认构造函数，则可以使用 `= default` 来声明并定义
6. 构造函数也会根据是否在类内定义决定其是否是 `inline` 函数
7. 若编译器支持类内初始值，则无须修改该值时最好不要在构造函数中初始化该值
8. 任何编译器合成的构造函数、赋值运算符、析构函数都隐式声明为 `inline`
9. 构造函数初始值列表的使用举例
...
类类型的成员会调用其拷贝构造函数来拷贝  
若定义了移动构造函数和和移动赋值运算符，则合成拷贝构造函数会被删除  
